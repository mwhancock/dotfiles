...
-- Sane text wrapping options for clean notes
-- Sane text wrapping options for clean notes
-- 1. Tell Neovim to load the downloaded packages into memory right now
vim.cmd('packadd bullets.vim')
vim.cmd('packadd autolist.nvim')
vim.cmd('packadd render-markdown.nvim')

-- 2. Run the initialization setups
require('autolist').setup()
require('render-markdown').setup({
  file_types = { 'markdown' },
})

-- 3. Sane visual configurations for clean Obsidian style
vim.opt_local.wrap = true
vim.opt_local.linebreak = true
vim.opt_local.conceallevel = 2  -- Hides raw characters (like `**`) so icons show

-- 4. Map the intelligent Tab shifts for bullet points
vim.keymap.set('i', '<Tab>', '<cmd>AutolistTab<cr>', { buffer = true, desc = "Autolist Tab" })
vim.keymap.set('i', '<S-Tab>', '<cmd>AutolistShiftTab<cr>', { buffer = true, desc = "Autolist Shift-Tab" })
